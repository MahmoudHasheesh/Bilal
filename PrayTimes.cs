...
namespace WpfApplication1.Coversion
{
    public class PrayTimes
    {
        ...
        public string[] timeSuffixes = { "am", "pm" };
	    ...
	    // Calculation Methods
	    methods = new Dictionary<MethodName, Method> {
            {
                MethodName.MWL,
                new Method
                {
                    name = "Muslim World League",
                    _params = new Dictionary<TimeName, object>{{ TimeName.Fajr, 18 }, { TimeName.Isha, 17 }}
                }
            },
            {
               MethodName.ISNA,
                new Method
                {
                    ...
                    _params = new Dictionary<TimeName, object> {{ TimeName.Fajr, 15 }, { TimeName.Isha, 15 }}
                }
            },
            {
                MethodName.Egypt,
                new Method
                {
                    ...
                    _params = new Dictionary<TimeName, object> {{ TimeName.Fajr, 19.5 }, { TimeName.Isha, 17.5 }}
                }
            },
            {
                MethodName.Makkah,
                new Method
                {
                    ...
                    _params = new Dictionary<TimeName, object> {{ TimeName.Fajr, 18.5 }, { TimeName.Isha, "90 min" }}
                }
            },  // fajr was 19 degrees before 1430 hijri
		    {
                ...
                new Method
                {
                    ...
                    _params = new Dictionary<TimeName, object> {{ TimeName.Fajr, 18 }, { TimeName.Isha, 18 }}
                }
            },
            {
                MethodName.Tehran,
                new Method
                {
                    ...
                    _params = new Dictionary<TimeName, object> {{ TimeName.Fajr, 17.7 }, { TimeName.Isha, 14 }, { TimeName.Maghrib, 4.5 }, { TimeName.Midnight, "Jafari" }}
                }
            },
            // isha is not explicitly specified in this method
		    {
                ...
                new Method
                {
                    ...
                    _params = new Dictionary<TimeName, object> {{ TimeName.Fajr, 16 }, { TimeName.Isha, 14 }, { TimeName.Maghrib, 4 }, { TimeName.Midnight, "Jafari" }}
                }
            }
	    };

        // Default Parameters in Calculation Methods
            defaultParams = new Dictionary<TimeName, object>
            {
        ...
        */
        ...
        setting = new Dictionary<TimeName, object>
        {  
            ...
            foreach (var method1 in methods)
            {
                ...
                var defParams = defaultParams.Where(p => !_paramss.ContainsKey(p.Key));
            ...
            // initialize settings
            var _params = methods[method]._params;
            foreach (var param in _params)
            {
                setting.Add(param.Key, param.Value);
            }

            // init time offsets

            offset = timeNames.ToDictionary(i => i, i => 0.0);
        }
        ...
        public void tune(double[] timeOffsets)
        {
            ...
            {
                var element = offset.ElementAt(i);
                offset[element.Key] = timeOffsets[i];
            }
        }


        // get current calculation method
        public MethodName getMethod() { return calcMethod; }

    // get current setting
    public Dictionary<TimeName, object> getSetting () { return setting; }

        // get current time offsets
        public Dictionary<TimeName, double> getOffsets() { return offset; }

        // get default calc parametrs
        public Dictionary<MethodName, Method> getDefaults() { return methods; }
        ...
        public Dictionary<TimeName, double> getTimes(DateTime date, GeoCoordinate coords, double? timezone = null, double? dst = null, string format = null)
    {
        ...
        lat = 1 * coords.Latitude;
        lng = 1 * coords.Longitude;
        elv = coords.Elevation.HasValue ? 1 * coords.Elevation.Value : 0;
        timeFormat = !string.IsNullOrEmpty(format) ? format : timeFormat;

        if (!timezone.HasValue)
            timezone = this.getTimeZone(date);
        if (!dst.HasValue)
            dst = this.getDst(date);
        timeZone = 1.0 * timezone.Value + (1.0 * dst.Value);
        jDate = this.julian(date.Year, date.Month, date.Day) - lng / (15 * 24);
        ...
        public string getFormattedTime(double time, object format, string[] suffixes = null)
    {
        ...
        var hour = (format == "24h") ? this.twoDigitsFormat((double)hours) : ((hours + 12 - 1) % 12 + 1).ToString();
        ...
        public Dictionary<TimeName, double> computePrayerTimes(Dictionary<TimeName, double> times)
    {
        ...
        return new Dictionary < TimeName, double> {
            { TimeName.Imsak, imsak}, { TimeName.Fajr, fajr}, { TimeName.Sunrise, sunrise}, { TimeName.Dhuhr, dhuhr},
            { TimeName.Asr, asr}, { TimeName.Sunset, sunset}, { TimeName.Maghrib, maghrib}, { TimeName.Isha, isha}
        ...
        public Dictionary<TimeName, double> computeTimes()
    {
        ...
        var times = new Dictionary<TimeName, double> {
            { TimeName.Imsak, 5}, {TimeName.Fajr, 5}, {TimeName.Sunrise, 6}, { TimeName.Dhuhr, 12},
            { TimeName.Asr,13}, { TimeName.Sunset,18}, { TimeName.Maghrib, 18}, { TimeName.Isha, 18}
    ...
    // add midnight time
    times[TimeName.Midnight] = (setting[TimeName.Midnight] == "Jafari") ? 
    ...
    public Dictionary<TimeName, double> adjustTimes(Dictionary<TimeName, double> times)
    {
        ...
        {
            times[times.ElementAt(i).Key] += timeZone - lng / 15;
        }

        if (paramss[TimeName.HighLats] != "None")
			times = this.adjustHighLats(times);

        if (this.isMin(paramss[TimeName.Imsak]))
            times[TimeName.Imsak] = times[TimeName.Fajr] - this.eval(paramss[TimeName.Imsak]) / 60;
        if (this.isMin(paramss[TimeName.Maghrib]))
            times[TimeName.Maghrib] = times[TimeName.Sunset] + this.eval(paramss[TimeName.Maghrib]) / 60;
        if (this.isMin(paramss[TimeName.Isha]))
            times[TimeName.Isha] = times[TimeName.Maghrib] + this.eval(paramss[TimeName.Isha]) / 60;
    ...
    {
        var factor = new Dictionary<AsrMethod, double> { { AsrMethod.Standard, 1}, { AsrMethod.Hanafi, 2}};
            //[asrParam];
    ...
    public Dictionary<TimeName, double> tuneTimes(Dictionary<TimeName, double> times)
    {
        for (int i = 0; i < times.Count; i++)
        {
            ...
            times[element.Key] += offset[element.Key] / 60.0;
        }
        return times;
    }
    ...
    public double adjustHLTime(double time, double _base, double angle, double night, object direction = null)
    {
        var portion = this.nightPortion(angle, night);
        var timeDiff = (direction == "ccw") ?
            this.timeDiff(time, _base) :
            this.timeDiff(_base, time);
        if (double.IsNaN(time) || timeDiff > portion)
            time = _base + (direction == "ccw" ? -portion : portion);
        return time;
    }
    ...
    {
        var method = setting[TimeName.HighLats];
    ...
    public double gmtOffset(DateTime date)
    {
        ...
        var GMTString = localDate.ToUniversalTime().ToString();
        var GMTDate = DateTime.Parse(GMTString.Substring(0, GMTString.LastIndexOf(" ") - 1));
        var hoursDiff = (localDate - GMTDate).TotalHours / (1000 * 60 * 60);
        return hoursDiff;
    }
    ...
    {
        var fdgdfg = Regex.Split(str + string.Empty, "[^0-9.+-]");
    ...
    {
        return (arg + string.Empty).IndexOf("min") != -1;
    }
